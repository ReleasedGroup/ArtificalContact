...
- The authenticated `/me` route includes the Sprint 3 composer preview panel for reusable post and reply variants, including local image drag/drop, thumbnail previews, and remove actions, plus a thread workspace that publishes real root posts to `POST /api/posts`.
...
- When `/p/{id}` resolves for an authenticated active user, the page also exposes a reply composer backed by `POST /api/posts/{id}/replies` and owner-only delete actions backed by `DELETE /api/posts/{id}`.
