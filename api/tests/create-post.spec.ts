...
describe('createPostHandler', () => {
  ...
  it('creates a media-only root post when the request includes attachments', async () => {
    const repository: PostRepository = {
      create: vi.fn(async (post) => post),
    }
    const handler = buildCreatePostHandler({
  ...
  it('returns validation errors when the post omits both text and media', async () => {
    const handler = buildCreatePostHandler({
      repositoryFactory: () => ({
        create: async (post) => post,
      }),
